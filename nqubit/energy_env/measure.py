...
def CalcuFidelity(n,p,Hb,Hp_array,T,g):
	...
	for i in range(lenthNumbers):
		...
		for i in range(int(T/g)+1): # the steps of iteration
				...
				C.append(c)
		#psi_1=psi.conjugate()
		#energy += abs(np.dot(psi,psi))
		#energy +=  np.dot(psi,Hp*psi)
		#print(energy)
		energy += -abs(np.dot(psi,Hp*psi)/np.dot(psi,psi)/lenthNumbers)
		#print(energy.shape())
		averageSuccess += C[-1]/lenthNumbers
	return energy,averageSuccess
...
if __name__ == '__main__':
	...
	energy, threshold =CalcuFidelity(n,p,Hb,Hp_array,T,g) 
	#print()
	#print(C)
